...
| 48  | **SOLD_OUT** → SweetAlert2 error blocks order creation                                 | If `stockInfo.status === 'SOLD_OUT'`, a red SweetAlert2 is shown: title "f534 Insufficient Quantity!", text from `stockInfo.message`, red confirm button (`#fa5252`), `allowOutsideClick: false`. The save is **blocked** — `return` prevents any further processing.                                                                                                                         |
| 49  | **INSUFFICIENT_STOCK with available > 0** → SweetAlert2 warning offers to cap quantity | If `stockInfo.status === 'INSUFFICIENT_STOCK'` and `availableStock > 0`, a warning SweetAlert2 is shown: title "f534 Insufficient Quantity!", text "{message}\n\nUse available quantity ({N}) to sell out?", confirm button "Use {N}", cancel button "Cancel", yellow confirm (`#fab005`). If confirmed, quantity is auto-updated to the available amount. If cancelled, save is **blocked**. |
...
| 51  | **LOW_STOCK** → SweetAlert2 warning shown but order proceeds                           | If `stockInfo.status === 'LOW_STOCK'`, a warning SweetAlert2 is shown: title "f7e1 Low Stock Warning", text from `stockInfo.message`, yellow confirm button (`#fab005`). Clicking OK allows the order to continue — **save is not blocked**.                                                                                                                                                  |
...
| 80  | Soft-delete cascades to linked payment records              | When `softDeleteAll` runs, associated payment records for the affected transactions are also soft-deleted to maintain integrity.                                   |
