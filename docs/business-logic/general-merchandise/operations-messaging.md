...
| 10  | Sending users can unsend their own messages with a permanent hard delete                                      | The sender delete action removes the message row entirely so it disappears for all participants.         |
| 11  | Receiving users can delete messages only from their own view                                                  | Receiver-side deletion creates a per-user visibility record so the message remains for other users.      |
| 12  | Conversation previews and unread counts ignore messages hidden by the current user                            | Last-message previews and unread badges are derived from only the messages visible to that operator.     |
| 13  | Any future GM-specific messaging persistence, routing, or operator actions must be documented here explicitly | This doc currently records real shared-workflow parity as the baseline.                                  |
| 14  | Shared messaging workflow changes that affect GM should also update this GM doc                               | Shared implementation still defines GM operator behavior.                                                |
