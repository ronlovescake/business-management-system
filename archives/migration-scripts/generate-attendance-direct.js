...
async function fetchSchedules() {
  return new Promise((resolve, reject) => {
    http
      .get('http://localhost:3000/api/schedules', (res) => {
        let data = '';
        res.on('data', (chunk) => {
          data += chunk;
        });
...
async function fetchLeaveRequests() {
  return new Promise((resolve, reject) => {
    http
      .get('http://localhost:3000/api/leave-requests', (res) => {
        let data = '';
        res.on('data', (chunk) => {
          data += chunk;
        });
...
async function generateAttendance() {
  try {
    ...
    for (const schedule of relevantSchedules) {
      ...
      const leaveInfo = leaveRequests.find((request) => {
        const requestId = String(request.employeeId || '')
          .trim()
          .toLowerCase();
        const scheduleId = String(schedule.employeeId || '')
          .trim()
          .toLowerCase();
        return (
      ...
      const attendanceRecord = {
        ...
        notes:
          onLeave && leaveInfo
            ? `Leave period: ${leaveInfo.startDate} to ${leaveInfo.endDate}`
            : null,
      };
    ...
    for (const empId of EMPLOYEES) {
      ...
      const empPresent = empRecords.filter(
        (r) => r.status === 'present'
      ).length;
      const empLeave = empRecords.filter((r) => r.status === 'on-leave').length;
      console.log(
        `  ${empId}: ${empRecords.length} total (${empPresent} present, ${empLeave} on leave)`
      );
    }
