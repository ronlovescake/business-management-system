...
import { Breadcrumbs, Button, Menu, Text, Group, ThemeIcon, Badge, ActionIcon } from '@mantine/core';
import { IconChevronDown, IconBuilding, IconUsers, IconSettings, IconHome, IconShirt, IconTruck } from '@tabler/icons-react';
...
export function BreadcrumbNavigation() {
  const { selectedBusiness, selectedWorkspace, setSelectedBusiness, setSelectedWorkspace, initializeFromPath } = useBusinessStore();
  ...
  const otherBusinesses = businesses.filter(b => b.value !== selectedBusiness);
    ...
    <Menu key="business" shadow="lg" width={280} radius="md">
      <Menu.Target>
        <Button 
          ...
          leftSection={currentBusiness ? (
            <ThemeIcon size="sm" radius="sm" variant="light" color={currentBusiness.value === 'clothing' ? 'pink' : 'blue'}>
              {currentBusiness.value === 'clothing' ? <IconShirt size={14} /> : <IconTruck size={14} />}
            </ThemeIcon>
          ) : (
        ...
        {currentBusiness ? (
          ...
          otherBusinesses.map((business) => (
            <Menu.Item
              ...
              leftSection={
                <ThemeIcon size="sm" radius="sm" variant="light" color={business.value === 'clothing' ? 'pink' : 'blue'}>
                  {business.value === 'clothing' ? <IconShirt size={14} /> : <IconTruck size={14} />}
                </ThemeIcon>
              }
            >
              {business.label}
      ...
      <Menu key="workspace" shadow="lg" width={220} radius="md">
        <Menu.Target>
          <Button 
            ...
            rightSection={<IconChevronDown size={14} />}
            leftSection={
              <ThemeIcon size="sm" radius="sm" variant="light" color={currentWorkspace?.value === 'operations' ? 'blue' : 'green'}>
            ...
            otherWorkspaces.map((workspace) => {
              ...
              return (
                <Menu.Item
                  ...
                  leftSection={
                    <ThemeIcon size="sm" radius="sm" variant="light" color={workspace.value === 'operations' ? 'blue' : 'green'}>
