...
export interface PostTemplateNoticeServiceLike {
  getPostTemplateNotice: () => Promise<PostTemplateNotice>;
  upsertPostTemplateNotice: (
    payload: Omit<PostTemplateNotice, 'id'>
  ) => Promise<PostTemplateNotice>;
}

export function createPostTemplateNoticeRoutes(
  service: PostTemplateNoticeServiceLike,
  domainLabel?: string
) {
  ...
  async function GET() {
    try {
      const notice = await service.getPostTemplateNotice();
      return ApiResponseUtil.success(notice);
    } catch (error) {
      logger.error(`Failed to load ${label}post template notice`, error);
  ...
  async function PUT(request: NextRequest) {
    try {
      ...
      const notice = await service.upsertPostTemplateNotice({
      ...
      logger.error(`Failed to update ${label}post template notice`, error);
      return NextResponse.json(
