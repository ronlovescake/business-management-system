...
import { ShippingResultSummary } from './ShippingResultSummary';
import { useShippingFeeCalculator } from '../hooks/useShippingFeeCalculator';

const HANDSONTABLE_THEME = `
  .ht-theme-horizon {
    --ht-font-size: 14px;
    --ht-line-height: 20px;
    --ht-font-weight: 400;
...
export function ShippingFeeCalculator({
  ...
  return (
    <Stack gap="md">
      <style>{HANDSONTABLE_THEME}</style>

      <ShippingCalculatorCard
        actualAlibabaShipping={actualAlibabaShipping}
        actualForwardersFee={actualForwardersFee}
        actualLalamove={actualLalamove}
        onAlibabaChange={(value) => setActualAlibabaShipping(value)}
        onForwardersChange={(value) => setActualForwardersFee(value)}
        onLalamoveChange={(value) => setActualLalamove(value)}
        shipmentCodes={shipmentCodes}
        selectedShipmentCode={selectedShipmentCode}
        onShipmentCodeChange={handleShipmentCodeChange}
      />

      <ShippingResultSummary summary={resultSummary} />
