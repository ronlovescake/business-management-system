...
type SummaryComparisonRowData =
  ...
  | {
      type: 'updated';
      row: SummaryUpdatedRow | null;
    }
  | null;

type SummaryComparisonTableRow = {
  key: string;
  label: string;
  type: 'added' | 'updated' | 'removed';
  snapshot: 'Before backup' | 'Current now';
  row: Record<string, unknown> | null;
...
interface BackupPreviewModalProps {
  ...
  summaryComparisonTableOptions: Array<{ value: string; label: string }>;
  summaryComparisonTable: string | null;
  summaryComparisonLoading: boolean;
  summaryComparison: BackupChangePreview | null;
  summaryComparisonError: string | null;
  summaryComparisonTypeOptions: Array<{
    value: 'added' | 'updated' | 'removed';
    label: string;
  }>;
  summaryComparisonType: 'added' | 'updated' | 'removed';
  summaryComparisonRowOptions: Array<{ value: string; label: string }>;
  summaryComparisonSelectedRows: string[];
  summaryComparisonSelectedRowsData: SummaryComparisonRowData[];
  ...
  onClose: () => void;
  onSelectSummaryComparisonTable: (table: string | null) => void;
  onSelectSummaryComparisonType: (
    value: 'added' | 'updated' | 'removed'
  ) => void;
  onSelectSummaryComparisonRows: (value: string[]) => void;
...
const renderComparisonTable = ({
  ...
  rows: SummaryComparisonTableRow[];
  columns: string[];
}) => (
  <Card withBorder padding="md" radius="md">
    <Stack gap="sm">
      <Text fw={600}>Selected row comparisons</Text>
      {rows.length ? (
        <Table.ScrollContainer minWidth={Math.max(columns.length * 160, 620)}>
            ...
            <Table.Tbody>
              {rows.map((row) => (
                <Table.Tr key={row.key}>
                  ...
                  <Table.Td>
                    <Badge
                      color={row.snapshot === 'Before backup' ? 'red' : 'green'}
                      variant="light"
                    >
                      {row.snapshot}
                    </Badge>
        ...
        <Alert color="gray">
          Select one or more rows to compare the backup snapshot against the
          current database using the full table structure.
        </Alert>
      )}
    </Stack>
  </Card>
...
const buildComparisonTableData = (rowDataList: SummaryComparisonRowData[]) => {
  const columns: string[] = [];
  const rows: SummaryComparisonTableRow[] = [];

  const includeColumns = (row: Record<string, unknown> | null) => {
  ...
  rowDataList.forEach((rowData, index) => {
    if (!rowData?.row) {
      return;
    }

    const beforeRow: Record<string, unknown> | null =
      rowData.type === 'updated'
        ? (rowData.row as SummaryUpdatedRow).backup
        : rowData.type === 'removed'
          ? (rowData.row as Record<string, unknown>)
          : null;
    const afterRow: Record<string, unknown> | null =
      rowData.type === 'updated'
        ? (rowData.row as SummaryUpdatedRow).current
        : rowData.type === 'added'
          ? (rowData.row as Record<string, unknown>)
          : null;
    const label =
      rowData.type === 'updated'
        ...
        : guessRowLabel(rowData.row ?? {}, `Row ${index + 1}`);
    const changedColumns =
      rowData.type === 'updated'
        ? Object.keys(rowData.row.changes)
        : Object.keys(rowData.row ?? {});

    includeColumns(beforeRow);
    includeColumns(afterRow);

    rows.push({
      key: `${label}-${index}-before`,
      label,
      type: rowData.type,
...
export const BackupPreviewModal = ({
  ...
  canDownloadDump,
  onClose,
  onSelectSummaryComparisonTable,
  onSelectSummaryComparisonType,
  onSelectSummaryComparisonRows,
  onSelectTable,
  onDownloadJSON,
  onDownloadDump,
  onDownloadAllCSV,
  ...
  const { columns: comparisonColumns, rows: comparisonRows } =
    buildComparisonTableData(summaryComparisonSelectedRowsData);
        ...
        <Tabs defaultValue="summary">
            ...
            <Stack gap="md">
              {previewData?.metadata ? (
                ...
                <Alert icon={<IconFileDescription size={16} />} color="gray">
                  ...
                  browsing is unavailable, but restore planning and dump
              ...
              <Card withBorder>
                <Stack gap="md">
                  <Group justify="space-between" align="flex-start">
                    <div>
                      <Title order={5}>Before / After Comparison</Title>
                      <Text size="sm" c="dimmed">
                        Compare multiple changed rows from one table using the
                        full column layout from the backup and the live
                        database.
                      </Text>
                    </div>
                    {summaryComparisonTable ? (
                  ...
                  <SimpleGrid cols={{ base: 1, md: 3 }} spacing="md">
                    <Select
                      label="Table"
                      placeholder={
                        summaryComparisonTableOptions.length
                          ? 'Select a table'
                          : 'No previewable tables'
                      }
                      data={summaryComparisonTableOptions}
                      value={summaryComparisonTable}
                      onChange={onSelectSummaryComparisonTable}
                      searchable
                      disabled={!summaryComparisonTableOptions.length}
                    />
                    <Select
                      label="Change type"
                      placeholder="Select a change type"
                      data={summaryComparisonTypeOptions}
                      value={summaryComparisonType}
                      onChange={(value) =>
                        onSelectSummaryComparisonType(
                          (value as 'added' | 'updated' | 'removed') || 'added'
                        )
                      }
                      disabled={summaryComparisonTypeOptions.length <= 1}
                    />
                    <MultiSelect
                      label="Row"
                      placeholder={
                        summaryComparisonRowOptions.length
                          ? 'Select one or more rows'
                          : 'No rows available'
                      }
                      data={summaryComparisonRowOptions}
                      value={summaryComparisonSelectedRows}
                      onChange={onSelectSummaryComparisonRows}
                      searchable
                      disabled={!summaryComparisonRowOptions.length}
                    />
                  </SimpleGrid>
                  ...
                  {summaryComparison ? (
                    <>
                      <Group gap="xs">
                        <Badge color="green">
                          {summaryComparison.addedCount} added
                        </Badge>
                        <Badge color="orange">
                          {summaryComparison.updatedCount} updated
                        </Badge>
                        <Badge color="red">
                          {summaryComparison.removedCount} removed
                        </Badge>
                        <Badge color="gray">
                          {summaryComparison.backupCount} backup /{' '}
                          {summaryComparison.currentCount} current
                        </Badge>
                      </Group>

                      {summaryComparisonSelectedRows.length ? (
                        <Text size="sm" c="dimmed">
                          Comparing {summaryComparisonSelectedRows.length} row
                          {summaryComparisonSelectedRows.length === 1
                            ? ''
                            : 's'}
                        </Text>
                      ) : null}
                      ...
                      {comparisonColumns.length > 0 ? (
                        ...
                        <Alert color="blue" icon={<IconArrowsDiff size={16} />}>
                          Select one or more rows to compare the backup snapshot
                          against the current database using the full table
                          structure.
                        </Alert>
                      )}
                    </>
          ...
          <Tabs.Panel value="changes" pt="md">
            <Stack gap="md">
              <Alert icon={<IconArrowsDiff size={16} />} color="blue">
                This compares the selected backup&apos;s saved row counts with
                the live database right now. It is a table-count change summary,
                not a row-by-row audit trail.
              </Alert>
                        ...
                        <Card withBorder>
                          <Stack gap="xs">
                            ...
                            <Text size="xs" c="dimmed">
                              ...
                              JSON-backed tables. Dump-only tables and very
                              large tables stay count-only.
                            </Text>
                          </Stack>
                        </Card>
                      ) : null}
          ...
          <Tabs.Panel value="tables" pt="md">
            {previewData ? (
              ...
              <Alert icon={<IconFileDescription size={16} />} color="gray">
                This backup cannot be browsed by table because only the
                PostgreSQL dump artifact is available.
              </Alert>
            )}
          </Tabs.Panel>
          ...
          <Tabs.Panel value="download" pt="md">
            <Stack gap="md">
              ...
              {previewData?.tables ? (
                ...
                <Alert icon={<IconFileDescription size={16} />} color="gray">
                  JSON, CSV, and XLSX inspection exports are unavailable for
                  this backup because no JSON inspection artifact was generated.
                </Alert>
              )}
            </Stack>
          </Tabs.Panel>
                          ...
                          >
                            <Stack gap={4}>
                              <Group justify="space-between">
                                ...
                                <Badge
                                  color={step.supported ? 'green' : 'gray'}
                                >
                                  {step.supported
                                    ? 'supported'
                                    : 'planned only'}
