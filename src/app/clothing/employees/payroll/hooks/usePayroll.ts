...
export function usePayroll(apiBasePath?: string) {
  ...
  const handleAddPayroll = async () => {
    ...
    try {
      ...
      const safeCount = Number.isFinite(count) ? count : 0;
      const selectedLabel =
        typeof normalized.period === 'object' && normalized.period !== null
          ? (normalized.period as {
              label?: string;
              start?: string;
              end?: string;
            })
          : null;
      const derivedLabel =
        selectedLabel?.label && selectedLabel.label.trim()
          ? selectedLabel.label
          : selectedLabel?.start && selectedLabel.end
            ? `${selectedLabel.start} to ${selectedLabel.end}`
            : payPeriodLabel || 'the selected period';

      const message =
        typeof normalized.message === 'string' && normalized.message.trim()
          ? normalized.message
          : `Successfully generated payroll for ${safeCount} employee${safeCount === 1 ? '' : 's'} (${derivedLabel}).`;
