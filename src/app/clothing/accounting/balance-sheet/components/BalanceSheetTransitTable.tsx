...
interface BalanceSheetTransitTableProps {
  rows: TransitBreakdownRow[];
  totalRows: number;
  summary: TransitBreakdownSummary;
  formatCurrency: (amount: number) => string;
}

export const BalanceSheetTransitTable = memo(function BalanceSheetTransitTable({
  rows,
  totalRows,
  summary,
  formatCurrency,
}: BalanceSheetTransitTableProps) {
  return (
    <Stack gap="md">
      <AccountingTableCard>
      ...
      <AccountingTableSummaryCard
        leftText={`Showing ${rows.length} of ${totalRows} transit detail rows`}
        items={[
          {
            label: 'Inventory in Transit Total:',
            value: formatCurrency(summary.inventoryInTransitBalance),
          },
          {
            label: 'Detail Sum:',
