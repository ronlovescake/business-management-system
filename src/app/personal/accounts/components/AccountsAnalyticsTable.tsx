...
import type { PersonalAccountRow } from './AccountsListTable';

interface AccountsAnalyticsTableProps {
...
export const AccountsAnalyticsTable = memo(function AccountsAnalyticsTable({
  accounts,
  formatCurrency,
}: AccountsAnalyticsTableProps) {
  const rows = useMemo(() => {
    const map = new Map<
      string,
    ...
    for (const a of accounts) {
      ...
      existing.count += 1;
      existing.total += a.balance || 0;
      map.set(key, existing);
    }

    const list = Array.from(map.values()).map((r) => ({
      ...r,
      percentage: totalBalance > 0 ? (r.total / totalBalance) * 100 : 0,
    }));
        ...
        <Table highlightOnHover withTableBorder style={{ minWidth: '900px' }}>
          ...
          <Table.Tbody>
            ...
            <Table.Tr style={{ backgroundColor: '#f8f9fa', fontWeight: 700 }}>
              ...
              <Table.Td style={{ textAlign: 'right' }}>
                <Text fw={700} size="lg" c="#495057">
                  {formatCurrency(rows.totalBalance)}
                </Text>
              </Table.Td>
            </Table.Tr>
          </Table.Tbody>
