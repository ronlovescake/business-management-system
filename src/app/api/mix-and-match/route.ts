...
export async function GET() {
  try {
    const rows = await prisma.bundleBatch.findMany({
      where: {
        bundleName: {
          startsWith: '[MIXMATCH] ',
        },
    ...
    logger.error('Failed to load mix & match rows', error);
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    const created = await prisma.bundleBatch.create({
      data: {
    ...
    logger.error('Failed to create mix & match row', error);
    const message =
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const existing = await prisma.bundleBatch.findUnique({ where: { id } });
    ...
    const updated = await prisma.bundleBatch.update({
      where: { id },
      data: {
        postingDate: parsed.postingDate,
        bundleName: toStoredMixAndMatchName(parsed.mixAndMatchName),
    ...
    logger.error('Failed to update mix & match row', error);
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const deleted = await prisma.$transaction(async (tx) => {
      const existing = await tx.bundleBatch.findUnique({ where: { id } });
    ...
    logger.error('Failed to delete mix & match row', error);
    return NextResponse.json(
