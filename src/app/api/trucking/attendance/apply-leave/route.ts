...
export async function POST(request: NextRequest) {
  try {
    ...
    if (existingDates.size > 0) {
      const updateResult = await attendanceModel.updateMany({
        where: {
          employeeId: employeeIdRaw,
    ...
    if (missingDates.length > 0) {
      const createResult = await attendanceModel.createMany({
    ...
    logger.error('Failed to apply leave to trucking attendance:', error);
    return NextResponse.json(
      {
