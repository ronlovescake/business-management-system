...
import {
  ...
  type TruckingEmployeeAutomationSettingsUpdate,
...
export async function GET() {
  try {
    await requireAdmin();
    const overview = await getTruckingEmployeeAutomationOverview();
    return NextResponse.json(overview);
  } catch (error) {
    logger.error(
      'Error fetching trucking employee automation settings:',
      error
    );
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const current = await getTruckingEmployeeAutomationSettings();
    const payload =
      (await request.json()) as TruckingEmployeeAutomationSettingsUpdate;
    const updated = await updateTruckingEmployeeAutomationSettings(payload);
    ...
    logger.error(
      'Error updating trucking employee automation settings:',
      error
    );
    return toErrorResponse(
      error,
  ...
  try {
    ...
    const result =
      automationType === 'payroll-generation'
        ? await executePayrollAutomation({ domain: 'trucking', settings })
        : await executeStayInAutomation({ domain: 'trucking', settings });

    const run = await createTruckingEmployeeAutomationRun(result, 'manual', {
      userId: user.id ?? null,
      userName: user.name ?? null,
    });
    ...
    logger.error('Error running trucking automation manually:', error);
    return toErrorResponse(
      error,
