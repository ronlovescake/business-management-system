...
/**
 * GET /api/accounting/recurring-payments/drafts
 */
export const GET = withErrorHandler(async (request: NextRequest) => {
  try {
    ...
    const validation = ClothingRecurringPaymentDraftListSchema.safeParse({
    ...
    const drafts = await clothingRecurringPaymentService.listDrafts(
      validation.data
    );

    return ApiResponse.success(drafts, 'Recurring payment drafts fetched');
  } catch (error) {
    logger.error('Failed to fetch recurring payment drafts', { error });
