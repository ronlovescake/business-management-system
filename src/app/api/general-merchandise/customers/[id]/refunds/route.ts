...
import type { InventoryBucket } from '@prisma/client';
...
function sanitizeCreateInput(input: TransactionRefundCreateInput):
  ...
  const reason =
    input.reason === undefined || input.reason === null
      ? null
      : sanitizers.notes(input.reason) || null;
...
export const POST = withErrorHandler<RouteContext>(
  async (request: NextRequest, context) => {
    ...
    if (!validation.success) {
      logger.warn('GM transaction refund validation failed', {
        customerId: idResult.id,
    ...
    logger.info('GM transaction refund recorded', {
      customerId: idResult.id,
      refundId: (created as { id?: number }).id,
    });

    return ApiResponse.success(created, 'Refund recorded');
  }
);
