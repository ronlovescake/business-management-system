...
export interface CalculateInvoiceWeightsResponse {
  ...
  invoices?: GeneralMerchandiseInvoice[];
  processedInvoiceCount: number;
}

type GeneralMerchandiseInvoice = Prisma.InvoiceGetPayload<{
  select: {
    id: true;
    customerName: true;
    createdAt: true;
  ...
  for (const invoice of invoices) {
    try {
      ...
      if (transactions.length === 0) {
        logger.warn(
          `No GM transactions found for customer: ${invoice.customerName}`
        );
        results.push({
      ...
      for (const transaction of transactions) {
        ...
        if (weightPerPiece !== undefined) {
          ...
          logger.warn(
            `No GM weight data found for product: ${productCode} (Customer: ${invoice.customerName})`
          );
        }
      }
      ...
      logger.error(
        `Error calculating GM weight for customer: ${invoice.customerName}`,
        error
      );
      results.push({
        customerName: invoice.customerName,
  ...
  groupedTransactions.forEach(({ displayName, items }) => {
    ...
    for (const transaction of items) {
      ...
      if (weightPerPiece !== undefined) {
        ...
        unmatchedProducts.push(productCode);
        logger.warn(
          `No GM weight data found for product: ${productCode} (Customer: ${displayName})`
        );
      }
    }
