...
async function findMissingEmployeeIds(
  ...
  const existing = await gmPrisma.generalMerchandiseEmployee.findMany({
    where: {
...
function handlePrismaError(error: unknown) {
  if (!(error instanceof Prisma.PrismaClientKnownRequestError)) {
    logger.error('Unexpected GM payroll Prisma error', { error });
  ...
  logger.error('Unhandled Prisma GM payroll error', {
    code: error.code,
    error,
  });
  return ApiResponse.error('Failed to process payroll request');
...
async function handleBulkPayload(payload: unknown[]) {
  ...
  logger.info('GM bulk payroll import complete', { count: created.length });
...
async function handleSinglePayload(body: unknown) {
  ...
  try {
    const payroll = await gmPrisma.generalMerchandisePayroll.create({
...
export const GET = withErrorHandler(async (request: NextRequest) => {
  ...
  const synced = toSync.length
    ? await syncPayrollDeductionsGeneralMerchandise(toSync)
    : [];
...
export const PUT = withErrorHandler(async (request: NextRequest) => {
  ...
  const existing = await gmPrisma.generalMerchandisePayroll.findUnique({
    where: { id },
  });
  ...
  try {
    ...
    logger.error('Failed to sanitize GM payroll update payload', { error });
  ...
  try {
    const payroll = await gmPrisma.generalMerchandisePayroll.update({
      where: { id },
      data: diff,
    });
    ...
    if (statusChangedToPaid) {
      const [syncedPayroll] = await syncPayrollDeductionsGeneralMerchandise([
        payroll,
      ]);
      const payrollForSync = syncedPayroll ?? payroll;

      logger.info('Syncing GM payroll to expenses', {
      ...
      try {
        ...
        logger.error('Failed to sync GM payroll to expenses', {
          error,
          payrollId: id,
        });
      }
...
export const DELETE = withErrorHandler(async (request: NextRequest) => {
  ...
  try {
    if (permanent) {
      await gmPrisma.generalMerchandisePayroll.delete({ where: { id } });
      logger.info('GM payroll permanently deleted', { id });
    } else {
      await gmPrisma.generalMerchandisePayroll.update({
        where: { id },
        data: { deletedAt: new Date() },
      });
      logger.info('GM payroll soft deleted', { id });
    }
