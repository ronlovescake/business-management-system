...
export const GET = withErrorHandler(
  async (_request: NextRequest, { params }: RouteContext) => {
    try {
      ...
      return ApiResponse.success(employee, 'Employee fetched');
    } catch (error) {
    ...
    try {
      ...
      const employeeData: Prisma.EmployeeUpdateInput = {
        ...
        currentSalary: body.currentSalary
          ? (sanitizers.number(body.currentSalary, { min: 0, decimals: 2 }) ??
            0)
          : body.basicSalary
            ? (sanitizers.number(body.basicSalary, { min: 0, decimals: 2 }) ??
              0)
            : 0,
        ...
        dateOfBirth: body.dateOfBirth
          ? sanitizers.date(body.dateOfBirth)
          : null,
        ...
        bankAccount: body.bankAccount
          ? sanitizers.name(body.bankAccount)
          : null,
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError) {
        if (error.code === 'P2002') {
          ...
          return ApiResponse.conflict(
            'Duplicate entry',
            `An employee with this ${target.join(', ')} already exists`,
            target[0]
          );
        }
