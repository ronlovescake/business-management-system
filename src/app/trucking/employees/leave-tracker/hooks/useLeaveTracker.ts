...
export default function useLeaveTracker() {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      await api.delete(`/api/trucking/leave-requests/${id}`);
    },
    onMutate: async (id) => {
  ...
  const saveMutation = useMutation({
    mutationFn: async (
      ...
      const response = await api.post('/api/trucking/leave-requests', payload);
      return response;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: leaveKeys.lists() });
  ...
  const approveMutation = useMutation({
    mutationFn: async (id: string) => {
      await api.patch('/api/trucking/leave-requests', {
        id,
        status: 'approved',
    ...
    onSuccess: async (id) => {
      ...
      if (
        ...
        try {
          await api.post('/api/trucking/attendance/apply-leave', {
