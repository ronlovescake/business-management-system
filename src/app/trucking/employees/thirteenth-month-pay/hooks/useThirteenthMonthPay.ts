...
export function useThirteenthMonthPay() {
    ...
    queryFn: async () => {
      const [activeEmployees, payrollRecords, persistedRecords] =
        await Promise.all([
          ...
          api.get<Array<Record<string, unknown>>>('/api/trucking/payroll'),
          (async () => {
            try {
  ...
  const editRecordMutation = useMutation({
    mutationFn: async ({
      ...
      await api.patch('/api/trucking/thirteenth-month-pay', payload);
  ...
  const deleteRecordMutation = useMutation({
    mutationFn: async (id: string) => {
      await api.delete(
        `/api/trucking/thirteenth-month-pay?recordId=${encodeURIComponent(id)}`
      );
      return id;
    },
